...
- **Test fix**: Created `backend/tests/conftest.py` with `client`, `admin_token_headers`, and `db` pytest fixtures; fixed 4 broken `test_import_export.py` tests → **48/48 tests now pass**
- **Security**: Removed wildcard `"*"` from `CORS_ORIGINS` in `config.py`
- **Quality**: `main.py` — admin seeding now only runs on first boot (no longer re-hashes password on every restart)
...
### Added (Analytics Dashboard — bonus module)
